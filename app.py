...
def processar_planilha_pro006(caminho_arquivo, linha_dados_str, tipo_pista, modo_calculo):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            sql_data = {
                'km': km_val, 'km_segmento': km_segmento, 'tipo_pista': tipo_pista, 'modo_calculo': modo_calculo,
        ...
        conn.commit()
        conn.close()
        print("Cérebro PRO-006 (V8) concluído. Banco de dados populado.")
        return True, None
...
def relatorio():
    ...
    try:
        ...
        query_segmentos = f"""
            SELECT 
                km_segmento, SUM(foi_inventariada) as N,
                AVG(fch_media_estaca) as media_fch, AVG(fch_var_estaca) as media_var,
                MIN(km) as km_inicio, MAX(km) + 0.020 as km_fim_corrigido,
                {select_defeitos_sql}
            FROM estacas
